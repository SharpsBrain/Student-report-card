...
void intro()
{
 ...
 void create_a_record()
 {
    ...
    outfile<<s.firstname<<"\t"<<s.secondname<<"\t"<<s.id<<"\t"<<s.pf<<"\t"<<s.gradepf<<"\t"<<s.ict<<"\t"<<s.gradeict<<"\t"<<s.eng<<"\t"<<s.gradeEng<<"\t"<<s.math<<"\t"<<s.grademath<<"\t"<<s.phy<<"\t"<<s.gradephy<<endl;
    outfile.close();
    cout<<endl;
 ...
 void viewall()
 {
    ...
    if(infile.fail())
    {
     ...
     cout<<"\t\t\t\t================================================"<<endl;
    while(infile>>s.firstname>>s.secondname>>s.id>>s.pf>>s.gradepf>>s.ict>>s.gradeict>>s.eng>>s.gradeEng>>s.math>>s.grademath>>s.phy>>s.gradephy)
    {
        cout<<"\t\t\t\tSTUDENT NAME          : "<<s.firstname<<' '<<s.secondname<<endl;
        cout<<"\t\t\t\tSTUDENT REG. NO       : "<<s.id<<endl;
...
void delete_record (string id)
{
    ...
    outfile<<s.firstname<<"\t"<<s.secondname<<"\t"<<s.id<<"\t"<<s.pf<<"\t"<<s.gradepf<<"\t"<<s.ict<<"\t"<<s.gradeict<<"\t"<<s.eng<<"\t"<<s.gradeEng<<"\t"<<s.math<<"\t"<<s.grademath<<"\t"<<s.phy<<"\t"<<s.gradephy<<endl;
        ...
        check = false;
    outfile1<<s.firstname<<"\t"<<s.secondname<<"\t"<<s.id<<"\t"<<s.pf<<"\t"<<s.gradepf<<"\t"<<s.ict<<"\t"<<s.gradeict<<"\t"<<s.eng<<"\t"<<s.gradeEng<<"\t"<<s.math<<"\t"<<s.grademath<<"\t"<<s.phy<<"\t"<<s.gradephy<<endl;       }
    }
    if (check)
    {
        cout<<"\t\t\t\tRECORD NOT FOUND..."<<endl;
...
void restore(string id)
{
    ...
    while (infile>>s.firstname>>s.secondname>>s.id>>s.pf>>s.gradepf>>s.ict>>s.gradeict>>s.eng>>s.gradeEng>>s.math>>s.grademath>>s.phy>>s.gradephy)
    {
        ...
        check = true;
         ...
         outfile2<<s.firstname<<"\t"<<s.secondname<<"\t"<<s.id<<"\t"<<s.pf<<"\t"<<s.gradepf<<"\t"<<s.ict<<"\t"<<s.gradeict<<"\t"<<s.eng<<"\t"<<s.gradeEng<<"\t"<<s.math<<"\t"<<s.grademath<<"\t"<<s.phy<<"\t"<<s.gradephy<<endl;
    }
    if (check)
    {
